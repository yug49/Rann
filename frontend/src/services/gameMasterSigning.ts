...
export interface YodhaTraitsData {
...
export class GameMasterSigningService {
  ...
  /**
   * Signs the Yodha traits and moves data for contract verification
   ...
   */
  async signTraitsAndMoves(data: YodhaTraitsData): Promise<`0x${string}`> {
    try {
      ...
      console.log('Game Master signed Yodha traits data:', {
      ...
      console.error('Error signing Yodha traits:', error);
      throw new Error(`Failed to sign Yodha traits: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

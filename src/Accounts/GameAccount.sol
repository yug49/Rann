...
//         keccak256("SessionKey(address account,uint256 validUntil,uint256 maxValue)");
...
//      */
//     function addSessionKey(address account, uint256 validUntil, uint256 maxValue, bytes calldata signature) external {
//         bytes32 structHash = keccak256(abi.encode(SESSION_KEY_TYPEHASH, account, validUntil, maxValue));
...
//         _sessionKeys[account] = SessionKey({validUntil: validUntil, maxValue: maxValue, isValid: true});
...
//         bytes32 recoveryHash = keccak256(abi.encodePacked("RECOVER_ACCOUNT", address(this), newOwner));

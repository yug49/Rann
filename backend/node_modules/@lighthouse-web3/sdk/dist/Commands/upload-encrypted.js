...
const uploadFile = async (path, signer, apiKey) => {
    const spinner = new cli_spinner_1.Spinner('Uploading...');
    spinner.start();
    const messageRequested = (await Lighthouse_1.default.getAuthMessage(getNetwork_1.config.get('LIGHTHOUSE_GLOBAL_PUBLICKEY'))).data.message;
    const signedMessage = await signer.signMessage(messageRequested);
    const uploadResponse = (await Lighthouse_1.default.uploadEncrypted(path, apiKey, getNetwork_1.config.get('LIGHTHOUSE_GLOBAL_PUBLICKEY'), signedMessage)).data;
    spinner.stop();
    process.stdout.clearLine(-1);
    process.stdout.cursorTo(0);
...
async function default_1(_path) {
    if (!_path) {
        console.log('lighthouse-web3 upload-encrypted <path>\r\n\r\n' +
        ...
        try {
            ...
            const signer = new ethers_1.ethers.Wallet(decryptedWallet.privateKey);
